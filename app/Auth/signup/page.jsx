...
const Signup = () => {
    ...
    async function handleLogin(event) {
        ...
        const response = await fetch(`http://192.168.1.2:2000/login/new`, { method: "POST", headers: { 'Accept': '*/*', 'Content-type': 'application/json' }, body: JSON.stringify({ username: uname.current.value, password: pass.current.value, name: pname.current.value }) })
        ...
        <div className="w-screen h-screen bg-zinc-800 flex flex-col select-auto pointer-events-auto">
            <div className="m-auto flex  h-[262.5px] rounded-lg border-[2px] border-zinc-700 w-[400px] mb-[5px] ">
                <form className="flex flex-col w-full h-full bg-zinc-700">
                    ...
                    <input type="text" placeholder="Name" className=" w-[90%] h-[50px] flex m-auto align-middle items-center rounded-lg justify-center text-center text-black" ref={pname} />
                    <input type="password" className=" h-[50px] w-[90%] flex m-auto align-middle items-center rounded-lg justify-center text-center text-black" placeholder="Password" ref={pass} />
                    {
